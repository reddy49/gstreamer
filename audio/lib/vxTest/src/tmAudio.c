...
/*
 * Copyright (c) 2015-2019, 2021 Wind River Systems, Inc.
 *
...
/*global*/
#if 1
LOCAL UINT32  sampleRate[] = {8000,11025,12000,16000,22050,24000,32000,44100,48000};     /* record sample rate */
...
VXTEST_STATUS tmWavFilesWriteRead(void);
VXTEST_STATUS tmWavFilesRecPlay(void);

/* functions */
void atInit()
    {
    (void)atCreateDisk(100);
    }
...
LOCAL STATUS atSetDir(char* dir)
    ...
    (void)snprintf(workDir, sizeof(workDir), "%s", dir);

    l = (UINT32)strlen(workDir);
...
LOCAL STATUS atWriteFile
    ...
    if(strlen(workDir))
        {
        (void)snprintf(fileName, AT_PATHLENGTH, "%s/Rec_%d_%d_%d.wav", workDir, sampleRate, sampleBits, channels);
        }
    else
        {
    ...
    audInfo.dataInfo.channels       = (UINT8)channels;
    ...
    while (dataSize > 0)
        ...
        while (readSize > 0)
            ...
            if (!block)   
                {
                bzero ((char *)&ioctlArg, sizeof (AUDIO_IOCTL_ARG));            
                if (OK != ioctl (audInfo.ad, AUDIO_FIONFREE, (char *)&ioctlArg))
                    {
...
LOCAL STATUS atRecWav
    ...
    while (dataSize > 0)
        ...
        if (!block)
            {
            bzero ((char *)&ioctlArg, sizeof (AUDIO_IOCTL_ARG));            
            if (OK != ioctl (audInfo.ad, AUDIO_FIONREAD, (char *)&ioctlArg))
                {
...
LOCAL BOOL atOptionCheck
    ...
    switch (option)
        ...
        case OPTION_BLOCK:
            bzero ((char *)&ioctlArg, sizeof (AUDIO_IOCTL_ARG));            
            if (OK != ioctl (ad, AUDIO_FIONFREE, (char *)&ioctlArg))
                {
                status = FALSE;             
                goto atperror;
                } 
        ...
        case OPTION_SAMPLEBITS:
            ...
            ioctlArg.dataInfo.sampleBits = (UINT8)value;
            if (OK != ioctl (ad, AUDIO_SET_DATA_INFO, (char *)&ioctlArg))
                {        
...
LOCAL STATUS atFileListRead(char *path)
    ...
    (void)atOpenDir(path, &audInfo.dd);

    /* open audio file and get the file information */
    while((audInfo.entry = atFindAudioFile(audInfo.dd))!=NULL)
        {
        (void)snprintf(fullpath, sizeof(fullpath), "%s/%s", path, audInfo.entry->d_name);
...
LOCAL STATUS atFileListRec(UINT32 recPeriod, UINT8 volume, BOOL block)
    ...
    for(i = 0; i < ARRAY_SIZE(sampleRate); i++)
        ...
        for(j = 0; j < ARRAY_SIZE(sampleBits); j++)
            {
            status = atRecWav(sampleRate[i], (UINT8)sampleBits[j], volume, recPeriod, 1, AUDIO_BUFTIME, block);
            if (AUDIOOK != status)
                {
                result = AUDIOERROR;
            ...
            status = atRecWav(sampleRate[i], (UINT8)sampleBits[j], volume, recPeriod, 2, AUDIO_BUFTIME, block);
            if (AUDIOOK != status)
                {
                result = AUDIOERROR;
                atprintf ("Rec fail status = %d\n", status);
...
LOCAL STATUS atDeviceIoctl(void)
    ...
    for(i = 0; i < ARRAY_SIZE(sampleRate); i++)
        ...
        for(j = 0; j < ARRAY_SIZE(sampleBits); j++)
            ...
            audInfo.dataInfo.sampleBits = (UINT8)(sampleBits[j]);
            audInfo.dataInfo.sampleBytes = (UINT8)(sampleBits[j] >> 3);
...
/*3*/
    ...
    if (ad >= 0)
        ...
        /*Coverity comment: test double close*/
...
VXTEST_STATUS tmWriteReadSpeed(UINT32 recPeriod)
    ...
    for (i = 0; i < ARRAY_SIZE(testList); i++)
        ...
        /* read */
        starttime = tickGet ();
        if ((readSize = read (audInfo.ad, (char *)buffer, dataSize)) < 0)
            atprintf ("read failed!\n");
        endtime = tickGet ();
        msecs = (endtime - starttime) / sysClkRateGet();

        atprintf("starttime:%ld  endtime:%ld Time\n", starttime, endtime);
        atprintf("Data Period:%ds  Read Period:%lds\n", recPeriod, msecs);
        ...
        atprintf("starttime:%ld  endtime:%ld Time\n", starttime, endtime);
        atprintf("Data Period:%ds  Write Period:%lds\n", recPeriod, msecs);
...
LOCAL VXTEST_ENTRY vxTestTbl_tmAudio[] = 
{
    ...
    {NULL, (FUNCPTR)"tmAudio", 0, 0, 0, 1000000, 0, 0, 0}
};
 
/**************************************************************************
*
* tmAudioExec - Exec test module
*
...
STATUS tmAudioExec
    ...
    {
    return vxTestRun((VXTEST_ENTRY**)&vxTestTbl_tmAudio, testCaseName, pTestResult);
    } 
...
STATUS tmAudioExec
    ...
    {
    return vxTestRun((VXTEST_ENTRY**)&vxTestTbl_tmAudio, testCaseName, pTestResult, argc, argv);
    }
...
int main
    ...
    {
    return tmAudioExec(NULL, NULL, argc, argv);
    }
#endif 
