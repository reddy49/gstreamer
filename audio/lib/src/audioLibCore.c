...
/*
 * Copyright (c) 2013-2016, 2019, 2021 Wind River Systems, Inc.
 *
...
LOCAL STATUS    audioScInit (void);
LOCAL STATUS    audioScMemValidate (const void * addr, size_t size,
                                    SC_PROT_ATTR access);
LOCAL STATUS    audioScIoctl (AUDIO_DATA * pDev, int function,
                              AUDIO_IOCTL_ARG *   pIoctlArg);
#endif /* _WRS_CONFIG_RTP */
LOCAL STATUS    audioIoctl (AUDIO_DATA * pDev, int function,
                            AUDIO_IOCTL_ARG *   pIoctlArg);
...
LOCAL STATUS    audioCoreIoctl (AUDIO_DATA * pDev, int function,
                                AUDIO_IOCTL_ARG * pIoctlArg, BOOL sysCall);
LOCAL STATUS    audioCoreIoctlDev (AUDIO_DATA * pDev, int function,
                                   AUDIO_IOCTL_ARG * pIoctlArg);
LOCAL STATUS    audioCoreIoctlChan (AUDIO_DATA * pDev, int function,
                                    AUDIO_IOCTL_ARG * pIoctlArg);
...
#ifdef _WRS_CONFIG_RTP
LOCAL const SC_IOCTL_TBL_ENTRY  ioctlAudioTbl[] =
    {
    {SC_IOCTL_ENABLE_WRITE, sizeof (AUDIO_IOCTL_ARG)}, /*  0: AUDIO_FIONREAD */
    {SC_IOCTL_ENABLE_WRITE, sizeof (AUDIO_IOCTL_ARG)}, /*  1: AUDIO_FIONFREE */
    {SC_IOCTL_ENABLE,       0                       }, /*  2: AUDIO_DEV_ENABLE */
    {SC_IOCTL_ENABLE,       0                       }, /*  3: AUDIO_DEV_DISABLE */
    {SC_IOCTL_ENABLE,       0                       }, /*  4: AUDIO_START */
    {SC_IOCTL_ENABLE,       0                       }, /*  5: AUDIO_STOP */
    {SC_IOCTL_ENABLE_READ,  sizeof (AUDIO_IOCTL_ARG)}, /*  6: AUDIO_SET_DATA_INFO */
    {SC_IOCTL_ENABLE_READ,  sizeof (AUDIO_IOCTL_ARG)}, /*  7: AUDIO_SET_VOLUME */
    {SC_IOCTL_ENABLE_READ,  sizeof (AUDIO_IOCTL_ARG)}, /*  8: AUDIO_SET_BUFTIME */
    {SC_IOCTL_ENABLE_READ,  sizeof (AUDIO_IOCTL_ARG)}, /*  9: AUDIO_SET_PATH */
    {SC_IOCTL_ENABLE_WRITE, sizeof (AUDIO_IOCTL_ARG)}, /* 10: AUDIO_GET_DEV_INFO */
    {SC_IOCTL_ENABLE_WRITE, sizeof (AUDIO_IOCTL_ARG)}, /* 11: AUDIO_GET_DATA_INFO */
    {SC_IOCTL_ENABLE_WRITE, sizeof (AUDIO_IOCTL_ARG)}, /* 12: AUDIO_GET_VOLUME */
    {SC_IOCTL_ENABLE_WRITE, sizeof (AUDIO_IOCTL_ARG)}, /* 13: AUDIO_GET_BUFTIME */
    {SC_IOCTL_ENABLE_WRITE, sizeof (AUDIO_IOCTL_ARG)}  /* 14: AUDIO_GET_PATH */
    };
#endif /* _WRS_CONFIG_RTP */
...
void audioCoreInit
    ...
    pAudioCore->ioSem = semMCreate (SEM_Q_FIFO | SEM_DELETE_SAFE);
...
#ifdef _WRS_CONFIG_RTP
    if (ERROR == audioScInit ())
        {
        AUDIO_CORE_DBG_MSG (100, "audioScInit failed.\n", 0, 0, 0, 0, 0, 0);
        goto error;
        }
#endif /* _WRS_CONFIG_RTP */
    ...
    pAudioCore->drvNum = iosDrvInstall (NULL,               /* creat() */
                            NULL,                           /* remove() */
                            ...
                            (DRV_IOCTL_PTR)audioIoctl);     /* ioctl() */

    if (ERROR == pAudioCore->drvNum)
        {
        goto error;
        }
    ...
    /*
     * Register a different ioctl function audioScIoctl() for system call
     * from RTP. It validates the address range and access permission.
     */

    if (iosDrvIoctlMemValSet (pAudioCore->drvNum,
                              (DRV_IOCTL_PTR) audioScIoctl) == ERROR)
        {
        goto error;
        }
#endif /* _WRS_CONFIG_RTP */
...
error:
    ...
    if (ERROR != pAudioCore->drvNum)
        {
        (void)iosDrvRemove (pAudioCore->drvNum, FALSE);
        }
...
STATUS audioCoreRegTransceiver
    ...
    pAudio->ioSem = semMCreate (SEM_Q_FIFO | SEM_DELETE_SAFE);
    ...
    if (ERROR == iosDevAdd ((DEV_HDR *)&pAudio->devHdr, devName,
                            pAudioCore->drvNum))
        {
        goto error;
        }
...
STATUS audioCoreUnregTransceiver
    ...
    if ((NULL != pAudio) && (0 == strcmp (pTail, "")))
        ...
        /*
         * Send task and receive task should be deleted before deleting message
         * queues, or else it may cause data abort exception.
         */
...
/*******************************************************************************
*
* audioScMemValidate - validate an address range passed to a system call routine
*
...
* Therefore, some address validation like the argument of AUDIO_DEV_ENABLE,
...
#ifdef _WRS_CONFIG_RTP
LOCAL STATUS audioScMemValidate
    (
    const void * addr,
    size_t       size,
    SC_PROT_ATTR access
    ...
    if (_func_scMemValidate != NULL)
        {
        return _func_scMemValidate (addr, size, access);
        }
    else
        {
        return ERROR;
...
/*******************************************************************************
*
* audioScIoctl - handle ioctl call from system RTP
*
* This is a wrapper routine of audioCoreIoctl() to handle ioctl from RTP.
*
* RETURNS: OK, or ERROR if failed to call audioCoreIoctl()
*
* ERRNO: N/A
*/

LOCAL STATUS audioScIoctl
    (
    AUDIO_DATA *        pDev,
    ...
    {
    return audioCoreIoctl (pDev, function, pIoctlArg, TRUE);
    }
#endif

/*******************************************************************************
*
* audioIoctl - handle ioctl call not from system RTP
*
* This is a wrapper routine of audioCoreIoctl() to handle ioctl not from RTP.
*
* RETURNS: OK, or ERROR if failed to call audioCoreIoctl()
*
* ERRNO: N/A
...
LOCAL STATUS audioIoctl
    ...
    {
    return audioCoreIoctl (pDev, function, pIoctlArg, FALSE);
    }
...
LOCAL STATUS audioCoreIoctl
    ...
    switch (function)
        {
        case AUDIO_FIONREAD: /* get num chars available to read */
            if (NULL == pIoctlArg)
                {
                status = ERROR;
                break;
            ...
            break;
        case AUDIO_FIONFREE: /* get free byte count on device */
            if (NULL == pIoctlArg)
                {
                status = ERROR;
                break;
        ...
        case AUDIO_DEV_ENABLE:
            if (AUDIO_STATE_IDLE == pDev->devState)
                ...
                if (NULL != pIoctlArg)
                    ...
                    if (sysCall)
                        {
                        status = audioScMemValidate ((const void *) pIoctlArg,
                                                     sizeof (AUDIO_IOCTL_ARG),
                                                     SC_PROT_READ);
                        if (status == ERROR)
                            {
                            break;
...
/*******************************************************************************
*
* audioScInit - audio ioctl() command validation initialization
*
* This routine implements the initialization of ioctl() comman validation for
* audio driver.
*
...
LOCAL STATUS audioScInit (void)
    {
    return scIoctlGroupRegister (VX_IOCG_AUDIO, ioctlAudioTbl,
                                 NELEMENTS (ioctlAudioTbl));
    }
#endif /* _WRS_CONFIG_RTP */
