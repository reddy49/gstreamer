...
void velm6030_drv_power(E_PWR_T pwr)
{
    ...
    //  VEML6030_I2C_READ(VEML6030_I2C_ADDR, CMD_CODE_ALS_CFG, &val,2);
    flexio_i2c_read(VEML6030_I2C_ADDR, CMD_CODE_ALS_CFG, (U8 *)&val, 2);
    ...
    flexio_i2c_write(VEML6030_I2C_ADDR, CMD_CODE_ALS_CFG, (U8 *)&val, 2);
...
void velm6030_drv_power_mode(E_STATUS_T pwrMode)
{
    ...
    flexio_i2c_write(VEML6030_I2C_ADDR, CMD_CODE_POWER_SAVING, (U8 *)&val, 2);
}

static S32 velm6030_drv_setting(S_VEML_CFG_T *cfg)
{
    flexio_i2c_write(VEML6030_I2C_ADDR, CMD_CODE_ALS_CFG, (U8 *)cfg, 2);
    return 0;
}
...
static S32 velm6030_drv_update_cfg(S_VEML_CCB *pstCcb, const S_VEML_RESOLUTION_TBL *res)
{
    ...
    return velm6030_drv_setting(&pstCcb->veml_cfg_reg);
}
...
int flexio_i2c_read(int addr, int reg, U8 *buff, int len)
{
	...
	printf("ioctl value ret in read = %d\n");
	perror("read i2c-2 failed");
        close(fd);
        return -1;
    }
...
int main(void)
{
        ...
        while(1)
	{
		...
		U32 als_value;
		S32 result = velm6030_drv_read_als(&als_value);

		if (result == STATUS_SUCCESS) {
			printf("ALS Value: %u\n", als_value);
				
		} else {
			printf("Failed to read ALS value\n");
		}
